        ...
        with st.spinner('正在渲染滿版畫布...'):
            try:
                ...
                if img_b64:
                    ...
                    function saveImage() {{
                        ...
                        html2canvas(area, {{ 
                            ...
                            canvas.toBlob(function(blob) {{
                                ...
                                link.href = url;
                                link.download = '質感圖卡_' + Math.floor(Date.now() / 1000) + '.png';
                    ...
                    <button onclick="saveImage()" class="download-btn">📸 下載高品質滿版圖卡 (PNG)</button>
                    """, unsafe_allow_html=True)
                st.balloons()
            except Exception as e:
                st.error(f"渲染失敗：{e}")
